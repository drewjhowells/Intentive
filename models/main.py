...
from transformers import BertTokenizer, BertModel, AutoTokenizer, AutoModelForCausalLM, TextStreamer
...
loaded_analyzer = EmotionAnalyzer.load("models/emotion_model.pth", "models/emotion_classes.json")
retriever = RAGRetriever()


def main():
    with open("data/primed_context.txt", "r", encoding="utf-8") as f:
        core_context = f.read()
    ...
    while conversation:
        ...
        total_input = agent._build_prompt_llm(user_input, emotions_str, rag_chunks)
        input_ids = tokenizer.encode(total_input, return_tensors="pt")
